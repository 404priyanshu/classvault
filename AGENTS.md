...
automated_test_suite: 199 Vitest tests (31 files), 29 Playwright smoke tests, 11 signed-in Playwright journeys, and 470 pgTAP tests across 20 suites run against the local Supabase stack with `supabase test db` (counts as of 2026-09-23)
implemented_routes:
  - path: /
...
- Free-form subjects work with public notes (2026-09-23). A subject the
  ...
  `find_or_create_subject`; migration
  `20260923020000_allow_public_notes_on_campus_subjects.sql` lets a public note
...
- Automated coverage as of 2026-09-23: 194 Vitest tests across 30 files
  ...
  Playwright smoke tests (`npm run test:e2e`, unauthenticated flows only), 11
...
12. There are no real users yet. As of 2026-09-22 production holds 3 accounts,
    ...
    The operator's own account (the one used for the repository) was granted
    `platform_admin` on 2026-09-23 so they can open `/dashboard/usage`; it is
    the only platform role in production. That role can also moderate notes
...
> with the production database matching the repository at 46 migrations.
